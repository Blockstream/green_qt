...
if [[ "$LAST_STEP" -lt $STEP_PREREQS ]]; then
    ...
    if [[ -z "$QT_ROOT" ]]; then
        ...
        for q in "$HOME/Qt/6.11.1/macos"; do
    ...
    if [[ -z "$QT_ROOT" || ! -f "$QT_ROOT/bin/qmake" ]]; then
        fail "Qt 6.11 not found. Install Qt from https://www.qt.io/download-qt-installer and set QT_ROOT, e.g.: export QT_ROOT=\$HOME/Qt/6.11.1/macos"
    fi
